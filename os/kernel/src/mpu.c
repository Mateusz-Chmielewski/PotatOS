...
void mpu_init()
{
  ...
  /*  MPU->RNR = 0x0; // set region 0
   ...
   MPU->RASR = (0 | MPU_REGION_SIZE_128B | 0x0 << 16 | 0x0 << 17 | 0x0 << 18 | 0x3 << 24 | 0x0 << 28);
  */
  /*   // core periphals
    MPU->RNR = 0x1;         // set region 1
    MPU->RBAR = 0xE000E000; // set base address
    MPU->RASR = (MPU_REGION_SIZE_128B) | (MPU_REGION_NO_ACCESS) | (MPU_REGION_NO_EXECUTE);
    // subregion 0

    // enable system control space
    MPU->RNR = 0x2;         // set region 2
    MPU->RBAR = 0xE000E000; // set base address
    MPU->RASR = (MPU_REGION_SIZE_1MB) | (MPU_REGION_FULL_ACCESS) | (MPU_REGION_NO_EXECUTE); */
