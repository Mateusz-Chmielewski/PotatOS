...
__attribute((naked)) void sv_call_handler(void)
{
  __asm volatile(
      ...
      "MRSNE r0, PSP              \n"    // If not equal, Process Stack Pointer (PSP)
      "B svc_handler_main            \n" // Branch to C function
  );
}

static void svc_handler_main(uint32_t *stack_frame)
{
  ...
  uint16_t svc_instruction = *((uint16_t *)(return_address - 2)); // Fetch SVC instruction
  ...
  case SYS_USART_WRITE:
    ...
    break;
  case SYS_USART_READ:
    usart_read((uart_handle_t *)stack_frame[0], (uint8_t *)stack_frame[1], (uint32_t)stack_frame[2]);
    break;
  default:
    break;
  }
...
void usart_syscall_read(struct uart_handle_t *uart_handle, char *buffer, uint32_t length)
{
  __asm volatile(
      "svc %0 \n"
      :
