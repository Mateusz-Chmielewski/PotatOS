...
/* PRIVDEFENA controls whether or not code running in Privileged thread mode can access the default memory map.
If the bit is set to 0, any access not explicitly enabled via the MPU will result in a fault when the ENABLE bit is set.
In a typical configuration using the MPU, this bit will be set to 1. */
#define MPU_CTRL_HFNMIENA BIT(1) // enable MPU during hard fault, NMI, and fault masked interrupt

#define MPU_CTRL_PRIVDEFENA BIT(2) // enable privileged access to default memory map
