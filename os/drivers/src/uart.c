...
void uart_init(struct uart_handle_t *uart_handle, unsigned long baud)
{
  ...
  uart_handle->uart->CR1 |= UART_CR1_UE | UART_CR1_RE | UART_CR1_TE; // Set UE (uart enable), RE (read enable), TE(transmision enable) bits
  uart_handle->uart->CR1 |= UART_CR1_RXNEIE;                         // Enable RXNE interrupt (interrupt on read)
